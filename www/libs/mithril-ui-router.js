    ...
    function mx_factory( m ) {
        ...
        mx.route.go = function( _state_ , _params_ ) {
            ...
            if( currentState ) {
                ...
                var onTrueExit = routes[currentState].onTrueExit;
                if( onExit ) { onExit(); }
            }
            ...
            splitState.forEach( function( partialState ) {
                ...
                if( onEnter ) { onEnter.bind(null, moduleRoot)(); }
                if (configuration.onExit)
                    configuration.onExit = configuration.onExit.bind(null, moduleRoot);
                if (configuration.onTrueExit) {
                    configuration.onTrueExit = configuration.onTrueExit.bind(null, moduleRoot, parentElem, nextElem);
                }
                if (onTrueExit)
                    onTrueExit();


                function modifyView(component) {
                    return function modifiedView() {
                        return bindToConfig(component.view.apply(this, arguments));
                    };
                }

                function bindToConfig(node) {
                    if ({}.toString.call(node).slice(8, -1) != 'Array') {
                        config = node.attrs.config;

                        node.attrs.config = customConfig;
                    }

                    return node;
                }

                function customConfig(elem, init, ctx) {
                    var output;

                    if (config)
                        output = config(elem, init, ctx);
